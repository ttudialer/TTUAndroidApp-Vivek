...
import com.kabaladigital.tingtingu.databinding.DthRechargeFragmentBinding;
...
import com.kabaladigital.tingtingu.viewmodels.DthRechargeViewModel;

public class DthRechargeFragment extends Fragment {

    private DthRechargeFragmentBinding binding;
    private DthRechargeViewModel mViewModel;
    private String select_Operator;

    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
                             @Nullable Bundle savedInstanceState) {
        binding = DataBindingUtil.inflate(inflater,R.layout.dth_recharge_fragment, container, false);


        select_Operator = PreferenceUtils.getInstance().getString(R.string.pref_recharge_select_dth_operator_name);
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        mViewModel = ViewModelProviders.of(this).get(DthRechargeViewModel.class);
    ...
    private boolean isValid(){
        ...
        if (binding.editMobileNumberOrVcNumber.getText().toString().length()<4){
        ...
        if (amount<=49){
            Toast.makeText(getActivity(), "Amount should be greater then or equal to 50", Toast.LENGTH_LONG).show();
            return false;
        }
